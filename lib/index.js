...
function createEngine(options) {
  ...
  // the globals. 
  engine.complete = function(input, cursorPos, cb) {
    var nextToken = acorn.tokenize(input);
    var token = {start: 0, end: 0};
    while (token.start >= cursorPos || token.end < cursorPos) {
      var token = nextToken();
      if (token.type.type == 'eof') {
        console.log("Error autocompleting, could not find token at current cursor position");
        return cb({
          token: {start: 0, end: 0},
          completions: []
        })
    ...
    var completions = _.filter(names, function(x) {
      return x.indexOf(token.value) === 0;
    });
    return cb({
      token: token,
